...
def execution_time_decorator(func):
    def wrapper(*args, **kwargs):
        ...
        print(f"{func.__name__}用时：{execution_time:.2f}s")
        return result
    return wrapper
...
if __name__ == '__main__':
    ###############################################
    input_log_path = '../data/input_log.txt'
    final_fp_path = '../data/final_fp'
    pickle_file = '../data/fingerprint_db.pickle'
    # pickle_file = f'fingerprint_db_{datetime.now():%Y%m%d%H%M}.pickle'
    ###############################################

    test_cases = load_log(input_log_path)
    fingerprint_db = load_fingerprint_db_all(final_fp_path, pickle_file) 
    # fingerprint_db = load_fingerprint_db(r'data\final_fp\final_fp_wenzhaoofficial_202406051522.csv') # 用于调试
    ...
    for idx, test_case in enumerate(test_cases):
        ...
        for i in range(len(test_case[1])):
            ...
            if case / 1e6 > 1 and str(case)[:2] in ['31','16']:
                ...
                break
        test_chunk_list = test_case[1][take_idx: take_idx+10]
        print(f"test_case:{test_chunk_list}")
        
        ret = main(test_chunk_list, fingerprint_db)
